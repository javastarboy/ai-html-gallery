...
const Badge = forwardRef<HTMLSpanElement, BadgeProps>(
  ({ className, variant = "default", source, color, children, ...props }, ref) => {
    const baseStyles = "inline-flex items-center justify-center h-6 px-3 text-tiny font-medium rounded-full";
    ...
    return (
      <span
        ...
        style={source && variant === "source" ? { backgroundColor: `${getSourceInfo(source).color}20`, color: getSourceInfo(source).color, borderColor: `${getSourceInfo(source).color}40` } : {}}
        {...props}
      >
